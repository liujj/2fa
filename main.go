...
func (c *Keychain) show(name string) {
	ok, err := touchid.Authenticate(fmt.Sprintf("access account %s", name))
	if err != nil {
		log.Fatalf("cannot access touchid module")
	}
	if !ok {
		log.Fatalf("invalid auth")
	}
	code := c.code(name)
	if *flagClip {
...
func (c *Keychain) showAll() {
	ok, err := touchid.Authenticate("access all account")
	if err != nil {
		log.Fatalf("cannot access touchid module")
	}
	if !ok {
